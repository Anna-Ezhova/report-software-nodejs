...
const DemountServerForm = ({ user }: UserName) => {
  const {
    register,
    handleSubmit,
    control,
    formState: { errors },
  } = useForm<TDemountServer>({
    resolver: zodResolver(DemountServer),
  });

  const fullErrors: FieldErrors<
    Extract<TDemountServer, { priority: "4" | "5" }>
  > = errors;
  ...
  /*  const {mutate, isLoading} = trpc.forms.demountServerRouter.useMutation({
        onError:(err) => {
...
export default DemountServerForm;
