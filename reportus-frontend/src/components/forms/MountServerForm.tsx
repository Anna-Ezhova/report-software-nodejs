...
const MountServerForm = ({ user }: UserName) => {
  const {
    register,
    handleSubmit,
    control,
    formState: { errors },
  } = useForm<TMountServer>({
    resolver: zodResolver(MountServer),
  });

  const fullErrors: FieldErrors<
    Extract<TMountServer, { priority: "4" | "5" }>
  > = errors;
  ...
  /* const {mutate, isLoading} = trpc.forms.mountServerRouter.useMutation({
        onError:(err) => {
...
export default MountServerForm;
